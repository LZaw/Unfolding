...
#include "CCFWillmoreTransformer.hpp"
...
CCFWillmoreTransformer::CCFWillmoreTransformer(UnfoldMesh* const m):
  CCFTransformer(m){
  updateProperties();
}
...
double CCFWillmoreTransformer::calculateEnergy(){
...
void CCFWillmoreTransformer::filter(){
  const Eigen::SparseMatrix<double>& M = m_mesh->getVertexMassMatrix();
  const Eigen::SparseMatrix<double> stepMatrix(m_cotMatrix * m_cotMatrix);
...
void CCFWillmoreTransformer::updateFilterSolver(){
  m_cotMatrix = m_mesh->getCotMatrix();
  std::tie(std::ignore, m_largestEigenValue) = DDGUtil::calculateLargestEigenpair(m_cotMatrix * m_cotMatrix);
  m_largestEigenValue = std::fabs(m_largestEigenValue);
  if(std::isnormal(m_largestEigenValue)){
    // Analyze Filterpattern
    m_filterSolver.analyzePattern(m_cotMatrix * m_cotMatrix);
